import type { Translations } from './nl'

export const en: Translations = {
  nav: {
    dashboard: 'Dashboard',
    tasks: 'Tasks',
    personnel: 'Personnel',
    analytics: 'Analytics',
    leaderboard: 'Ranking',
    settings: 'Settings',
  },
  dashboard: {
    title: 'Today',
    activeTasks: 'Active tasks',
    completedTasks: 'Completed tasks',
    noActiveTasks: 'No active tasks',
    noCompletedTasks: 'No completed tasks today',
    startTask: 'Start task',
    endTask: 'End task',
    refresh: 'Refresh',
    lastUpdated: 'Last updated',
    notUpdatedYet: 'Not updated yet',
    averagePerformance: 'Average performance',
    todaySummary: "Today's summary",
    tasksCompleted: 'tasks completed',
    minutesAhead: 'minutes ahead',
    minutesBehind: 'minutes behind',
    onTime: 'On time',
    currentlyWorking: 'Currently working',
    noOneWorking: 'Nobody working',
  },
  tasks: {
    title: 'Tasks',
    allTasks: 'All tasks',
    startNew: 'Start new task',
    department: 'Department',
    colli: 'Colli',
    expected: 'Expected',
    actual: 'Actual',
    difference: 'Difference',
    started: 'Started',
    ended: 'Ended',
    status: 'Status',
    active: 'Active',
    completed: 'Completed',
    personnel: 'Personnel',
    notes: 'Notes',
    noTasks: 'No tasks found',
    duration: 'Duration',
    sessionCount: 'Sessions',
    editTask: 'Edit task',
    deleteTask: 'Delete task',
    confirmDelete: 'Delete this task? This cannot be undone.',
    colliCount: 'Colli count',
    overview: 'Overview',
    inProgress: 'In progress',
    finished: 'Finished',
    timeWorked: 'Time worked',
    planned: 'Planned',
    team: 'Team',
    busy: 'Busy',
    done: 'Done',
    actions: 'Actions',
    editCompletedHint: 'Only notes and end time can be changed for completed tasks',
    startTime: 'Start time',
    startTimesPerPerson: 'Start time per person',
    endTime: 'End time',
    endTimesPerPerson: 'End time per person',
    invalidTimeRange: 'End time must be after start time',
    saveChanges: 'Save',
    noNotes: 'No notes',
    discountContainerBadge: 'Discount',
    discountContainerTitle: 'Discount container',
    discountContainerDescription: 'This task is a discount container, so its expected duration is calculated shorter than a normal container.',
    discountContainerNote: 'There is no fixed time limit for discount containers yet. We are reviewing the data and working toward realistic expectations, so the expected duration is mainly indicative for now.',
  },
  taskForm: {
    title: 'Start task',
    selectPersonnel: 'Select personnel',
    department: 'Department',
    selectDepartment: 'Select department',
    discountContainer: 'Is this a discount container?',
    discountContainerHint: 'Discount containers automatically get a shorter expected duration.',
    discountContainerInfoDescription: 'Choose whether this task is a discount container. Discount containers are estimated shorter than normal containers.',
    discountContainerYes: 'Yes, discount',
    discountContainerNo: 'No, normal',
    colliCount: 'Colli count',
    colliPlaceholder: 'e.g. 40',
    notes: 'Notes (optional)',
    notesPlaceholder: 'Additional remarks...',
    expectedMinutes: 'Expected duration',
    startTask: 'Start task',
    cancel: 'Cancel',
    minutes: 'minutes',
  },
  personnel: {
    title: 'Personnel',
    addNew: 'Add',
    fullName: 'Full name',
    namePlaceholder: 'First and last name',
    status: 'Status',
    active: 'Active',
    inactive: 'Inactive',
    notes: 'Notes',
    notesPlaceholder: 'Optional notes...',
    save: 'Save',
    cancel: 'Cancel',
    edit: 'Edit',
    deactivate: 'Deactivate',
    activate: 'Activate',
    noPersonnel: 'No personnel found',
    addFirst: 'Add the first person to get started.',
    confirmDeactivate: 'Are you sure you want to deactivate this person?',
    sessions: 'Sessions',
    lastActive: 'Last active',
    never: 'Never',
    addPersonnel: 'Add personnel',
    editPersonnel: 'Edit personnel',
    totalTasks: 'Total tasks',
    avgPerformance: 'Avg. performance',
    performanceHistory: 'Performance history',
    noHistory: 'No completed tasks yet',
    searchPlaceholder: 'Search...',
    avgPerColli: 'Avg. per colli',
    last7d: 'Last 7 days',
    last14d: 'Last 14 days',
    last30d: 'Last 30 days',
    last90d: 'Last 90 days',
    allTime: 'All time',
    customRange: 'Custom',
    noDataForPeriod: 'No data for selected period',
    filters: 'Filters',
    recentTasks: 'Recent tasks',
    showAllTasks: 'Show all tasks',
    comments: 'Comments',
    showAllComments: 'Show all comments',
    noComments: 'No comments yet',
    addComment: 'Add comment',
    commentPlaceholder: 'Write a comment...',
    deleteComment: 'Delete',
    confirmDeleteComment: 'Delete comment?',
    avgRating: 'Avg. rating',
    noRatings: 'No ratings yet',
    allTasksTitle: 'All tasks',
    allCommentsTitle: 'All comments',
  },
  ratings: {
    title: 'Rating',
    skipAll: 'Skip',
    skip: 'Skip',
    submit: 'Save',
    next: 'Next',
    personOf: 'of',
    workEthicLabel: 'Effort',
    workEthicHint: 'How motivated and hard-working was this person?',
    qualityLabel: 'Quality',
    qualityHint: 'How accurate and thorough was the work delivered?',
    teamworkLabel: 'Teamwork',
    teamworkHint: 'How well did this person work with others?',
    commentLabel: 'Additional comment (optional)',
    commentPlaceholder: 'Optional comment...',
    addComment: 'Add comment',
    tapToRate: 'Tap to rate',
    overallScore: 'Overall',
    showDetails: 'Show details',
    detailsTitle: 'Rating details',
    noRatingDetails: 'No ratings yet',
  },
  analytics: {
    title: 'Analytics',
    filters: 'Filters',
    dateFrom: 'Date from',
    dateTo: 'Date to',
    allPersonnel: 'All personnel',
    allDepartments: 'All departments',
    apply: 'Apply',
    reset: 'Reset',
    overview: 'Overview',
    totalSessions: 'Total sessions',
    completedSessions: 'Completed sessions',
    avgExpected: 'Avg. expected',
    avgActual: 'Avg. actual',
    avgDifference: 'Avg. difference',
    byPersonnel: 'By personnel',
    byDepartment: 'By department',
    dailyOverview: 'Daily overview',
    noData: 'No data for the selected period',
    sessionCount: 'Sessions',
    name: 'Name',
    department: 'Department',
    date: 'Date',
    minutes: 'minutes',
  },
  settings: {
    title: 'Settings',
    language: 'Language',
    languageDescription: 'Choose the display language of the application.',
    dutch: 'Dutch',
    english: 'English',
    saved: 'Saved',
  },
  common: {
    loading: 'Loading...',
    error: 'An error occurred',
    retry: 'Try again',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    back: 'Back',
    confirm: 'Confirm',
    yes: 'Yes',
    no: 'No',
    search: 'Search',
    filter: 'Filter',
    noResults: 'No results',
    minutes: 'min',
    today: 'Today',
    unknown: 'Unknown',
    loadMore: 'Load more',
    vsLastPeriod: 'vs. prev. period',
    filters: 'Filters',
  },
  validation: {
    required: 'This field is required',
    minLength: 'Minimum {min} characters required',
    maxLength: 'Maximum {max} characters allowed',
    minValue: 'Value must be at least {min}',
    maxValue: 'Value may be at most {max}',
    invalidEmail: 'Invalid email address',
    invalidUuid: 'Invalid ID',
  },
  welcome: {
    tagline: 'Supermarket filler performance tracking',
    featureLeaderboard: 'Ranking',
    featureAnalytics: 'Analytics',
    featureTasks: 'Tasks',
    featureLeaderboardDesc: 'View performance scores for all staff.',
    featureAnalyticsDesc: 'Insight into actual vs expected times.',
    featureTasksDesc: 'Track tasks and sessions in real time.',
    signIn: 'Sign in',
    continueWithout: 'Continue without signing in',
    hint: 'Sign in for full access or view the leaderboard.',
  },
  signIn: {
    username: 'Username',
    password: 'Password',
    submit: 'Sign in',
    invalidCredentials: 'Username or password is invalid.',
    viewLeaderboard: 'View leaderboard',
  },
  resetPassword: {
    title: 'Set password',
    newPassword: 'New password',
    confirmPassword: 'Confirm password',
    submit: 'Save password',
    signingInAs: 'Signing in as',
    invalidToken: 'This password link is invalid or expired.',
    passwordTooShort: 'Password must be at least 8 characters.',
    passwordMismatch: 'Passwords do not match.',
    updateFailed: 'Password could not be updated.',
  },
}
